...
//    }
    ...
    public static Map<LatLng, Tuple<LatLng, Period>> CreatePointsWithPeriod(Integer quantityPairs){
        ...
        while (pointMap.size() < quantityPairs){
        	...
        	if (!pointMap.containsKey(origin)){    
        		...
        		Tuple<LatLng, Period> destination = getDestinationWithPeriod(origin, 0);
    ...
    static LatLng getRandomPoint() {
    	return new LatLng(-23.54839 + .09 * (Math.random() * 2 - 1), -46.64035 + .09 * (Math.random() * 2 - 1));
    }
    
    
    static Tuple<LatLng, Period> getDestinationWithPeriod(LatLng origin, int cycle) {
    	LatLng destination = getRandomPoint();        		
		Period period = getRouteTimeFromServiceRoute(origin, destination);
		if (period != null)
		{
			return new Tuple<LatLng, Period>(destination, new Period());        
		}
		...
		// recursao
		getDestinationWithPeriod(origin, ++cycle);
